...
- 🔴 **三道退回原文的保險，使用者的話絕不能消失**：呼叫失敗、空回覆、
  ...
  極短文字（<4 字）直接返回，省一次呼叫。
- ⚠️ **翻譯頁籤那套語音（`pStartVoice`／`_pVoiceSR`／`pCleanText`，約 6074 行起）仍是舊做法**：
  Web Speech ＋ 寫死 `lang='zh-TW'` ＋ 寫死中文 toast，**在中國不可用、非中文使用者不可用**。
  這次沒有動它（不同功能、使用者沒要求）。要修的話：辨識換成呼叫 `transcribe`，
  整理沿用它自己的 `pCleanText` 即可。
