...
- ⚠️ **實際遇到的無聲來源：macOS 的「接續互通」**（2026-09-06，使用者自己查出來的）。
  Mac 把 iPhone 當成麥克風輸入時，瀏覽器抓到的是靜音裝置 —— 錄得到檔案、有位元數、
  但音量接近零，Whisper 就填了樣板句。**這類問題我們改不了程式碼，只能明確告知**，
  所以 `wav` 那行 log 會直接寫出「← 幾乎無聲」或「← 有收到聲音」的結論。
- 🔴 **診斷 log 一律攤平成單行字串，不要傳物件**（v2.1 修）。
  Chrome 的 console 會把物件收成 `Object`，要點開才看得到值 —— 而這幾行存在的
  唯一目的就是「請使用者把數字貼給我」。物件版實際造成使用者貼過來一串
  沒有任何資訊的 `Object`。**加新的診斷輸出時照這個寫法。**
...
「偶而翻譯失敗」不是隨機，是三件必然會發生的事疊在一起。修法都在 `lcTranslate`／
`lcTranslateAndSave`：

- 🔴 **輸出格式改成 XML 標籤，不用 JSON。** JSON 在這裡有一個必然的失敗模式：
  多行訊息的換行必須轉義成 `\n`，而模型常常直接輸出實體換行 —— `JSON.parse` 就丟錯，
  整則訊息的譯文全部掉失。標籤對換行、引號、反斜線一律免疫。
  Portal 自己的「翻譯」工具（`pRunTranslate`）本來就是用標籤，同一個理由。
  **不要為了「比較好解析」改回 JSON。**
- 🔴 **`max_tokens` 必須隨訊息長度成長**（`text.length * 8 + 400`，下限 1200、上限 16000）。
  輸出是原文的**四份**：寫死 1500 的話 300 字中文就已經在邊緣，500 字必定被截斷，
  而截斷的回覆缺尾巴的標籤 → 永遠解析失敗。**這是主因。**
- 🔴 **三次重試 ＋ 退避**（`LC_TR_TRIES`／`LC_TR_BACKOFF`）：claude-proxy 冷啟、
  edge function 併發上限、429 都是「一次就失敗但隔一下就過」（同 `SB.get` 的重試理由）。
- **完整性檢查**：畫面顯示的是「原文以外的三語」，所以那三個都要有內容才算成功；
  缺一個就當失敗重試（截斷的回覆正是缺尾巴那一兩個），錯誤訊息會指出缺哪幾語。
- 🔴 **孤兒訊息由開啟者補翻**（`LC_ORPHAN_MS` = 60 秒）：翻譯是**送出者的瀏覽器**發動的，
...
- 🔴 **房間裡用「本場次首次發言順序」配色，不是工號雜湊。**
  雜湊的問題不是函式選得不好，是方法本身：12 色配 4 個人，任兩人同色的機率是
  `1 − 12·11·10·9/12⁴ ≈ 43%`，那是**完美雜湊的理論下限** ——
  換 fnv1a、加 finalizer 都改變不了（實測 51 個真實工號，djb2 已達 43.3%）。
  而「不同人不同色」正是這個需求唯一的要求。
- **配好就快取（`lcColorMap`）**：新人加入只拿下一個顏色，**既有的人不換色**。
  若改成「依工號排序取位置」，每來一個新人就會把排在他後面的人整批位移。
  換場次時 `lcColorReset()`（`lcOpen`／`lcCreate`／`lcBackToList` 三處都有）。
- **清單頁仍用工號雜湊 `lcHue()`**：那裡同時列出很多場次的開啟者，沒有「這一場的發言順序」
...
- 🔴 **辨識完之後一定要接 `lcCleanTranscript()` 做 AI 整理**（v1.88）。
  這是使用者指出的關鍵：**「🌐 翻譯」頁籤的語音輸入之所以「效果好、標點調得好」，
  不是它的辨識比較準** —— 那邊用的是同一套 Web Speech API，而且 `lang` 寫死 `'zh-TW'`
  （越南同事用會全錯）。好的原因全部來自 `sr.onend` 之後呼叫 `pCleanText()`，
  把逐字稿送給 Claude 整理標點與流暢度。
  whisper 的輸出雖然已有標點，但不會去掉「嗯」「那個」這類口語填充、也不會修順語句。
  兩者疊加（whisper 辨識 ＋ Claude 整理）比任一個單獨都好。
- 🔴 **整理的最大風險是「整理」變成「改寫」**：線上對話的內容接著會被翻成四語、
  存進資料庫、匯出成 PDF —— 動到語意就是竄改別人的發言。
  所以 `LC_CLEAN_SYS` 比翻譯頁籤那一句嚴格得多：同語言、不翻譯、不增刪、
  不回答、不得比原話更客氣、專有名詞與數字原樣保留。輸出包在 `<clean></clean>`
  裡（避免模型加前言，同 `lcTranslate` 改用標籤的理由）。
- 🔴 **三道退回原文的保險，使用者的話絕不能消失**：呼叫失敗、空回覆、
  以及**結果長度不足原文六成**（模型很可能自己摘要或截斷了，那比沒整理更糟）。
  極短文字（<4 字）直接返回，省一次呼叫。
- ✅ **翻譯頁籤那套語音已經換掉了**（2026-09-26 查證）：`pStartVoice` 現在走
  `ComartVoice.toggle()`（＝共用引擎 → `transcribe`），`lang:''` 自動判斷語言，
  錯誤訊息走五語的 `lc_mic_*`。整份 `index.html` 已 grep 不到
  `webkitSpeechRecognition` 與 `_pVoiceSR`。
  🔴 **這一段以前寫著「仍是舊做法、在中國不可用」，而那早就不成立了。**
  過期的警告比沒有警告更糟 —— 它會讓人去修一個已經修好的東西，
  或反過來以為東莞廠不能用這個功能。**發現哪一段過期就當場改掉。**

### 時區標註（v1.84）

- 🔴 **訊息時間是用每位觀看者自己的當地時間顯示的**，所以越南同事（UTC+7）與台灣同事（UTC+8）
  看同一則訊息會差一小時。那本來就是對的，但**不標時區會讀起來像資料錯誤**；
  匯出的 PDF 更是把匯出者的時區烙進檔案裡。
- `lcTzLabel()` 回 `UTC+08:00 (Asia/Taipei)`。房間標頭用 i18n 的 `lc_tz_note`（`{tz}` 佔位），
  **PDF 表頭固定繁中**（同 `siteLabelZ` 的道理）。

### 其他

- 🔴 **PDF 匯出：寫最終內容前必須 `w.document.open()`**（v1.84 修）。
  前面寫過「正在準備文件…」的佔位文字而文件還沒 `close`，這時直接 `write` 是**附加**在後面，
  那行字就留在 PDF 上了。`document.open()` 會把整份文件清空重來。
  （佔位文字本身是必要的：圖片轉 data URL 要等，空白視窗看起來像壞掉。）
- **輸入區 `rows="3"`、`min-height:72px`、自動長高上限 `LC_INPUT_MAX = 220px`**（v1.84 前是 132px）。
- **「開新對話」的主題欄按 Enter 不會開案**（v1.83，只 `preventDefault`）：
  必須按下按鈕才建立場次。打主題時順手按 Enter 就開了一場空對話，是實際發生過的誤觸。
...
- **寫入防護**（sb-proxy）：`CHAT_HOST_ONLY`（`status`／`keep`／`title`）比對 `host_emp_id` 與簽章 empId；
  `host_emp_id`／`id` 在 `CHAT_IMMUTABLE` 完全禁改。
  `last_at` **刻意不保護**——每個人發言都要更新它，那是正常的協作寫入。
  讀取層仍是前端判斷（同 premortem，這是已接受的取捨）。
- 🔴 **刪除整場：開啟者本人或 admin（`CHAT_DELETE_ROLES`），這與事前驗屍相反。**
  驗屍是「只有主席刪得掉，admin 沒有例外」——那是因為匿名承諾要靠它撐住。
  線上對話沒有匿名，而**沒人收掉的房間會永遠掛在「進行中」**，需要有人能清。
  角色走 `liveRoleOf()`（重新查資料庫，不讀簽章裡的 role：降權後舊 token 還在有效期內），
  停用／離職者一律不算。該 helper 由這裡與受限 bucket 守衛共用（v1.81 從後者抽出，
  兩處問的是同一個問題，各留一份必然分岔）。
...
### 🔗 邀請連結：連結＝通行證（v2.04，2026-09-24，migration 202609240001）

房間工具列「結束對話」左邊多一顆 **🔗 邀請連結**（發起人、進行中才有）。
按下複製 `?lc=<場次id>&j=<通行碼>`，**收到的人點開就真的加入這場對話**。

- 🔴 **這推翻了 v1.98 的「連結不是通行證，只是直達的指標」**（使用者 2026-09-24 定案）。
  舊規則下邀請制的連結打不開，所以刻意不給那顆鈕；現在連結帶通行碼，
  伺服器驗過就把人併進 `members`，公開與邀請制**都給**這顆鈕。
- 🔴 **為什麼一定要 `join_code`，不能只憑場次 id**：
  **admin 的清單看得到每一場的 id**（他要能判斷哪一場該刪），而
  「admin 讀不到別人的對話」是這個功能最核心的承諾。只憑 id 就能自助加入的話，
  他把 id 貼進網址就繞過去了。通行碼讓「看得到 id」與「拿得到連結」變成兩件事。
- 🔴 **`join_code` 只有發起人拿得到**：sb-proxy 對 `chat_sessions` 的回應，
  `host_emp_id !== 簽章身分` 的列一律剝掉這個欄位。
  **這道剝除刻意不限於 GET** —— 每個人發言都會 PATCH `last_at`，
  帶 `return=representation` 時回的也是整列。
- 🔴 **加入這件事三道全在伺服器端**（`chatJoin` 分支）：
  ① 比對 `join_code`；② `status` 必須是 `open`（已結束的是存檔，不該事後混進人）；
  ③ **`members` 由 sb-proxy 自己組**（舊名單 ∪ 簽章身分），不採用前端送來的陣列 ——
  兩個人同時點連結會互相覆寫，而且那等於把「誰在名單裡」交還給前端決定。
  body 只能有 `join_code` 一個 key，混在其他欄位裡一律 403（那是想繞過 `CHAT_HOST_ONLY`）。
- **前端只送 `{join_code}`**（`lcJoinByLink`）。失敗**照樣往下開房間**：
  他可能本來就是參與人（自己按自己發的連結、或名單裡的人點到舊連結），
  真正的門仍是 `lcOpen` 的 `lcCanSee`。只有 `chat_closed` 會單獨說一句 ——
  否則使用者只看到「你不在參與人中」，對不上他的處境。
- **`lcGoto(id, joinCode)` 第二個參數可省略**：`shared/invite.js` 的邀請卡走一個參數
  （被邀請的人本來就在名單裡），不要為了它去改共用模組。
- 「這裡的規則」第 2 條五語跟著改了：邀請制現在是「參與人，**以及發起人把連結給了誰**」。
  那不是文案，是使用者要不要在裡面講真話的依據。
- **按鈕只給發起人**（`live && lcIsHost()`）：`members` 本來就只有他改得動。
  要開放給房裡所有人只是改一個條件 —— 但那等於任何參與人都能拉人進來。

### 參與人：只有參與人能開啟（v1.85，migration 202609050003）

`chat_sessions.members`（`text[]`）。建立時挑人，開啟者事後可「＋ 邀請」加人。
**原本是「單一全公司大廳」（2026-09-05 早上的決定），當日下午改成這個。**

- 🔴 **`members` 不含開啟者本人**：身分已在 `host_emp_id`，存兩份就會有
  「從 members 移除自己」這種矛盾狀態要處理。判斷一律經 `lcCanSee()`。
- 🔴 **admin 可以刪除任何一場，但讀不到內容。** 這是刻意的不對稱（使用者 2026-09-05 指定）：
  admin 在清單上只看得到標題／開啟者／人數／時間這些中介資料（不然無從判斷哪一場該刪），
  那些列標 🔒 且點不開。**不要「順手統一」讓 admin 也能開啟** —— 那等於讓管理者
  讀得到全公司的私下對話。
- 🔴 **一般使用者的清單查詢在資料庫端就過濾**（`host_emp_id=eq.<我>` 與
  `members=cs.{<我>}` 兩個查詢再合併），**不是撈全部再前端隱藏** ——
  後者會把全公司的對話標題送進每個人的瀏覽器。
  用兩個查詢而不是 PostgREST 的 `or=(...)`：`or` 裡面要包 `cs.{...}` 的大括號與引號，
  跨版本轉義行為不一致。
- **舊場次的 `members` 是 `'{}'`，即「只有開啟者看得到」** —— 刻意的保守預設：
  舊資料沒有參與人資訊，猜錯的方向該是「看不到」而不是「全公司看得到」。
- `members` 已加入 sb-proxy 的 `CHAT_HOST_ONLY`：任何參與者都能自行加人的話，
  這條規則就沒有意義了。
- ⚠️ 這仍是 UI 層與載入層的守衛，不是資料層的牆（同 premortem 的既有取捨）。

### 圖片附件（v1.82，migration 202609050002）

`chat_messages.img_path`／`img_name`／`img_mime`／`img_w`／`img_h`，檔案存 **`chat-images`**
（`public=false`）。可用 📎 鈕、**貼上**（Ctrl+V，截圖最常用）或拖放；一次多張＝一張一則訊息。
輸入框有字時當成圖說與圖片同一則送出。

- 🔴 **bucket 是私有的，與 board 的 `poll-images`（公開）刻意不同。** 投票圖片是「選項示意圖」，
  內容可預期；線上對話貼的是什麼無法預期（實務上多半是訂單、報價、BOM 的截圖），而**公開
  bucket 的網址一旦被轉發出去就永久有效、不需登入**。與成本檔案採同一個判斷。
  代價：`<img src>` 不能直接指過去，要先向 sb-proxy 換簽章網址。
- 🔴 **`chat-images` 刻意不加入 `RESTRICTED_BUCKETS`**：那份清單會把存取限縮到 admin／dcc，
  而對話圖片必須全體同事都看得到。不在清單上 ＝ 任何持有有效 session 的人都能經 sb-proxy 取用，
  正是要的權限。**不要「順手」把它加進去。**
- 🔴 **簽章網址快取在 `lcImgUrl`（1 小時，過期前 5 分鐘才重換）**：聊天室每 3 秒重繪一次，
  每次都重簽等於每 3 秒對每張圖打一次請求。已快取的在 `lcMsgHtml` 就直接帶上 `src`
  （沒帶的才由 `lcHydrateImages()` 補），否則每次重繪圖片都會閃一下。
- 🔴 **物件鍵一律純 ASCII**（`chat/<場次>/<uid>.<ext>`）：Supabase Storage 的物件鍵不接受中文
  （實測回 400 InvalidKey），而截圖檔名幾乎都是中文——這是報價系統 BOM 檔踩過的同一個坑。
  原始檔名存 `img_name`，只用於顯示。
- 🔴 **GIF 一律原檔上傳，不經 canvas 重新編碼**（那會把動畫壓成一張靜態圖）。
  其餘：長邊 ≤1800px 且 ≤1.5MB 的原檔上傳（**PNG 截圖轉 JPEG 會讓文字邊緣起毛邊，
  而聊天室最常貼的就是文字截圖**），超過才縮到 1800px 轉 JPEG q0.85。
- **`img_w`／`img_h` 存下來是為了 `aspect-ratio` 預留版面**：聊天室會自動捲到底，
  圖片載入後才撐開高度會讓正在讀的人畫面跳動。
- **只貼圖不打字**（`text=''`）是合法狀態：不送翻譯、不顯示「翻譯中…」、譯文區塊整塊不畫。
- **PDF 匯出把圖片內嵌成 data URL**，且 🔴 **`window.open` 必須在第一個 `await` 之前呼叫** ——
  await 之後就脫離使用者手勢，會被彈出視窗封鎖器擋掉。抓不到的圖印成「［圖片：檔名］」，
  不讓整份匯出失敗。
- 上傳失敗或訊息寫入失敗時**不清空輸入框、不寫訊息、解開 `lcBusy`**。
  圖已上傳但訊息沒進資料庫時**不刪那個孤兒物件**：刪除也可能失敗，而 bucket 是私有的、
  沒有任何訊息指向它，留著只佔一點空間。
