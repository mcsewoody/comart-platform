...
const navigation = [
  ...
  { to: "/manual-upload", label: "手動上傳", icon: UploadCloud, uploadOnly: true },
...
export function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0E17] text-slate-100">
      ...
      {mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 bg-slate-950/70" aria-label="關閉選單" onClick={() => setMobileOpen(false)} /><aside className="relative h-full w-[84%] max-w-xs bg-[#111827] p-4 shadow-2xl"><div className="mb-5 flex items-center justify-between"><p className="font-black text-white">文件庫</p><button className="rounded-lg p-2 hover:bg-slate-800" onClick={() => setMobileOpen(false)} aria-label="關閉選單"><X size={20} /></button></div><Navigation onNavigate={() => setMobileOpen(false)} /></aside></div>}
    </div>
  );
}
...
function Navigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <nav aria-label="主要導覽" className="space-y-1">{navigation.map((item) => {
    ...
    return <NavLink key={item.to} to={item.to} end={item.end} onClick={onNavigate} className={({ isActive }) => cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition", isActive ? "bg-cyan-400 text-white" : "text-slate-400 hover:bg-slate-800 hover:text-white")}><Icon size={18} />{item.label}</NavLink>;
  })}</nav>;
}
