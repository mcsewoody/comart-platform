...
export function PilotUploadPage() {
  ...
  return <>
    ...
    <Card className="p-5 md:p-6">
      ...
      {files.length > 0 && <><div className="mt-5 flex flex-wrap gap-3"><Badge tone="accent">自製品 {mfg.length}</Badge><Badge tone="accent">外購品 {buy.length}</Badge><Badge>{formatBytes(files.reduce((sum, item) => sum + item.file.size, 0))}</Badge></div><div className="mt-4 max-h-[420px] overflow-auto rounded-xl border border-slate-700">{files.map((item) => <div key={`${item.relativePath}-${item.file.size}`} className="grid gap-2 border-b border-slate-800 px-4 py-3 text-sm last:border-0 md:grid-cols-[90px_minmax(0,1fr)_170px]"><span className={item.dataset === "mfg" ? "text-cyan-300" : "text-amber-300"}>{item.dataset === "mfg" ? "自製品" : "外購品"}</span><span className="truncate text-slate-200" title={item.relativePath}>{item.relativePath}</span><span className="text-xs text-slate-500">{item.status}</span></div>)}</div></>}
